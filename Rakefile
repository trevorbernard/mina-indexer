...
def rustfmt_nightly?
  stdout, = Open3.capture3('rustfmt --version | grep stable || echo "true"')
  stdout.strip == 'true'
end

def nightly_if_required
  rustfmt_nightly? ? '' : '+nightly'
end
...
def cmd_output(cmd)
  ...
  raise "Command '#{cmd}' failed with exit status #{$CHILD_STATUS.exitstatus}" unless $CHILD_STATUS.success?
...
desc 'Lint all code'
task lint: %i[lint_ruby lint_shell lint_nix cargo_machete lint_rust]
...
namespace :test do
  ...
  tier2_regression_tests = %w[load_v1 load_v2 best_chain_many_blocks all]
  ...
  task tier2: ['test:unit:tier2'] do
    ...
    tier2_regression_tests.map do |test|
